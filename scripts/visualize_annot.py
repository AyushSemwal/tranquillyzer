...
def visualize_sequence_annotations(colors, read_name, read,
    ...
    for page_start in range(0, num_chunks, max_chunks_per_page):
        ...
        fig, axs = plt.subplots(num_page_chunks + 1, 1,
        ...
        axs[0].text(0.5, 0.5, wrapped_header,
                    ha='center', va='center',
        ...
        for ax_idx, (read_chunk, label_chunk) in enumerate(zip(page_read_chunks, page_label_chunks), start=1):
            ...
            for idx, (base, label) in enumerate(zip(read_chunk, label_chunk)):
                if label:
                    ax.text(idx / chars_per_line, 1, base,
                            ha='center', va='center',
                            color=colors[label],
                ...
                if current_label != label or idx == len(read_chunk) - 1:  # End of section or end of chunk
                    if current_label:
                        ...
                        ax.text(label_position, 0.5, current_label,
                                ha='center', va='center',
...
def save_plots_to_pdf(sequences, annotated_reads,
                      read_names, filename, colors,
                      chars_per_line=100,
                      max_chunks_per_page=50):
    with PdfPages(filename) as pdf:
        for sequence, annotated_read, read_name in zip(sequences, annotated_reads, read_names):
            ...
            figures = visualize_sequence_annotations(colors, read_name,
                                                     sequence,
                                                     predicted_labels,
                                                     architecture, reason,
                                                     chars_per_line,
