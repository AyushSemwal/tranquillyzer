...
# Parse CB / UMI from QNAME
def extract_cb_umi(read_name: str):
    parts = read_name.split("_")
    if len(parts) >= 3:
        # Return (CB, UMI, cleaned_name_without_CB_UMI)
        return parts[1], parts[2], "_".join(parts[:1] + parts[3:])
...
def process_region(
    ...
    """
    deduper = Deduper(umi_ld=umi_ld, per_cell=per_cell, stranded=stranded, position_tolerance=10)
